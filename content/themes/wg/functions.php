...
function foxhound_fonts_url() {
	 ...
	 */
	$alegreya = _x( 'on', 'Roboto Slab font: on or off', 'foxhound' );

	/*
	 * Translators: If there are characters in your language that are not
	 * supported by Roboto Slab Sans, translate this to 'off'. Do not translate into
	 * your own language.
	 */
	$alegreya_sans = _x( 'on', 'Roboto Slab Sans font: on or off', 'foxhound' );

	/*
	 * Translators: If there are characters in your language that are not
	 * supported by Roboto Slab SC, translate this to 'off'. Do not translate into
	 * your own language.
	 */
	$alegreya_sc = _x( 'on', 'Roboto Slab SC (smallcaps) font: on or off', 'foxhound' );

	if ( 'off' !== $alegreya || 'off' !== $alegreya_sans || 'off' !== $alegreya_sc ) {
		$font_families = array();

		if ( 'off' !== $alegreya ) {
			$font_families[] = rawurlencode( 'Roboto Slab:400,400italic,700,700italic,900italic' );
		}

		if ( 'off' !== $alegreya_sans ) {
			$font_families[] = rawurlencode( 'Roboto Slab Sans:700' );
		}

		if ( 'off' !== $alegreya_sc ) {
			$font_families[] = rawurlencode( 'Roboto Slab SC:700' );
		}
